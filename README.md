...
- This fork carries a [`build-bootstraps`](.github/workflows/build-bootstraps.yml) workflow that drives `termux/termux-packages` with `TERMUX_APP__PACKAGE_NAME=com.termux.hs.shortcut` so a bootstrap rebuild bakes the prefix `/data/data/com.termux.hs.shortcut/files/usr` into every binary. The workflow runs automatically on pull requests that modify it and on pushes to `master` that modify it, and is also available via `workflow_dispatch`. The `termux-am` Gradle subbuild inside the upstream `termux/termux-packages` docker image needs to run `sdkmanager` to install `platforms;android-33` and `build-tools;30.0.3`, which the restricted AppArmor profile loaded by upstream `scripts/run-docker.sh` denies; the workflow therefore rewrites the `--security-opt apparmor=_custom-termux-package-builder-…` argument to `--security-opt apparmor=unconfined` in `SEC_OPT` before invoking the docker container. `unconfined` is used instead of simply stripping the opt because docker's default profile otherwise blocks the `fuse-overlayfs` mount that the upstream build needs to layer the NDK. Disabling that profile is safe in this CI context because the AppArmor sandbox only confines the ephemeral build container on the GitHub Actions runner, it does not affect the produced bootstrap zips or the APK, and the `pull_request` trigger from a fork runs without secrets and with a read-only `GITHUB_TOKEN`. To bound supply-chain risk from the relaxed container, `TERMUX_PACKAGES_REF` defaults to the full commit SHA `3bb9b00f220e04508656c04a70f78eba68569503` (the commit currently pointed at by the upstream tag `bootstrap-2026.05.17-r1+apt.android-7`) so a tag-retag attack on the upstream tag cannot land arbitrary code in the build container. The release job that publishes the bootstrap zips runs only on `push` to `master` and `workflow_dispatch`, never on `pull_request`, so fork PRs never attempt to write a Release. Until the workflow produces a usable zip end-to-end, `app/build.gradle` continues to download the upstream `com.termux` bootstrap and the runtime path-patching in `TermuxInstaller` / `TermuxShellEnvironment` rewrites references at install / exec time.
