...
Commit messages must use the [Conventional Commits](https://www.conventionalcommits.org) spec. The CI enforces commit types via commitlint (`.github/workflows/configs/commitlint.config.js`). The space after the colon `:` is necessary. For a breaking change, add an exclamation mark `!` before the colon `:`.
...
This fork accepts **two sets of commit types**:

**Lowercase standard Conventional Commits types** — use these for CI, documentation, maintenance, and infrastructure changes:
...
- `autogen` — auto-generated changes
- `prep` — preparation commits
- `adapt` — adaptation commits

**Uppercase Keep-a-Changelog types** — use these for user-visible changes that should appear in the changelog. Changelogs are generated via the [`create-conventional-changelog`](https://github.com/termux/create-conventional-changelog) script. The description should be in present tense. You can optionally add a scope, like `Fixed(terminal): Fix some bug`.
...
For example: `fix(bootstrap): resolve permission error`, `Added: Add foo feature`, `Changed!: Change baz as a breaking change`.
##
...
- This fork carries a [`build-bootstraps`](.github/workflows/build-bootstraps.yml) workflow that drives `termux/termux-packages` with `TERMUX_APP__PACKAGE_NAME=com.termux.hs.shortcut` so a future bootstrap rebuild can bake the prefix `/data/data/com.termux.hs.shortcut/files/usr` into every binary. The workflow is currently `workflow_dispatch` only and is not yet producing usable bootstrap zips — the `termux-am` Gradle subbuild inside the upstream `termux/termux-packages` docker image fails to install `platforms;android-33` and `build-tools;30.0.3` because the restricted AppArmor profile denies writes to `/home/builder/lib/`. Until that is resolved, `app/build.gradle` continues to download the upstream `com.termux` bootstrap and the runtime path-patching in `TermuxInstaller` / `TermuxShellEnvironment` rewrites references at install / exec time.
