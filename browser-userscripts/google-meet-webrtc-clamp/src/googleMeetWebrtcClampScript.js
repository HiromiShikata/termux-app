...
const DEFAULT_MAX_FRAMERATE = 15;
const DEFAULT_MEET_ORIGIN = 'https://meet.google.com';

function defaultGoogleMeetWebrtcClampConfig() {
  return {
    enabled: DEFAULT_ENABLED,
    maxWidth: DEFAULT_MAX_WIDTH,
    maxHeight: DEFAULT_MAX_HEIGHT,
    maxFramerate: DEFAULT_MAX_FRAMERATE,
    meetOrigin: DEFAULT_MEET_ORIGIN,
  };
}

function buildGoogleMeetWebrtcClampScript(config) {
  const resolved = Object.assign(defaultGoogleMeetWebrtcClampConfig(), config || {});
  return '(function(){try{'
    + 'if(window.__termuxMeetWebrtcClampApplied){return;}'
    + 'window.__termuxMeetWebrtcClampApplied=true;'
    + 'var ENABLED=' + (resolved.enabled ? 'true' : 'false') + ';'
    + 'var MAX_WIDTH=' + resolved.maxWidth + ';'
    + 'var MAX_HEIGHT=' + resolved.maxHeight + ';'
    + 'var MAX_FRAMERATE=' + resolved.maxFramerate + ';'
    + 'var MEET_ORIGIN=' + JSON.stringify(resolved.meetOrigin) + ';'
    + 'if(!ENABLED){return;}'
    + 'if(location.origin!==MEET_ORIGIN){return;}'
    ...
    + 'function buildClampedEncodings(existingEncodings,track){'
    + 'var scale=computeScaleDownBy(track);'
    + 'var encodings=existingEncodings&&existingEncodings.length?existingEncodings:[{}];'
    + 'var next=[];'
    + 'for(var i=0;i<encodings.length;i++){'
    + 'var encoding={};for(var key in encodings[i]){encoding[key]=encodings[i][key];}'
    ...
    + 'parameters.encodings=buildClampedEncodings(parameters.encodings,sender.track);'
    ...
    + '}catch(e){}}'
    + 'function clampTransceiverInitEncodings(init,track){try{'
    + 'if(!init||typeof init!=='+ "'object'" +'){return init;}'
    + 'if(!track||track.kind!=='+ "'video'" +'){return init;}'
    + 'var next={};for(var key in init){next[key]=init[key];}'
    + 'next.sendEncodings=buildClampedEncodings(init.sendEncodings,track);'
    + 'return next;}catch(e){return init;}}'
    ...
    + 'var clampedInit=clampTransceiverInitEncodings(init,'
    + 'trackOrKind&&trackOrKind.kind?trackOrKind:null);'
    + 'var transceiver=originalAddTransceiver.call(pc,trackOrKind,clampedInit);'
